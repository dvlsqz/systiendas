...
class CartController extends Controller
{
    ...
    public function postCartAdd($id, Request $request){
        if(is_null($request->input('inventory'))):
            return back()->with('messages', 'Seleccione una opción del producto.')
                    ->with('typealert', 'danger');
        else:
            $inventory = Inventory::where('id',$request->input('inventory'))->count(); 

            if($inventory == "0"):
                return back()->with('messages', 'La opción seleccionada no esta disponible.')
                        ->with('typealert', 'danger');
            else:
                $inventory = Inventory::find($request->input('inventory'));

                if($inventory->product_id != $id):
                    return back()->with('messages', 'No podemos agregar este producto al carrito.')
                        ->with('typealert', 'danger');
                else:            
                    $order = $this->getUserOrder();
                    ...
                    if($request->input('quantity') < 1):
                        return back()->with('messages', 'Es necesario ingresar la cantidad que desea ordenar de este producto.')
                            ->with('typealert', 'danger');
                    else:
                        ...
                        if($inventory->limited == "0"):
                            ...
                            if($request->input('quantity') > $inventory->quantity):
                                return back()->with('messages', 'No disponemos de esa cantidad en inventario en este momento.')
                                ->with('typealert', 'danger');
                            endif;
                        ...
                        if(count(collect($inventory->getVariants)) > 0 ):
                            ...
                            if(is_null($request->input('variant'))):
                                return back()->with('messages', 'Seleccione una de las opciones de este producto.')
                                ->with('typealert', 'danger');
                            endif;
                        ...
                        if(!is_null($request->input('variant'))):
                            ...
                            if($variant == "0"):
                                return back()->with('messages', 'Selección no encontrada.')
                                    ->with('typealert', 'danger');
                            else:
                                $variant = Variant::find($request->input('variant'));

                                if($variant->inventory_id != $inventory->id):
                                    return back()->with('messages', 'Selección no valida.')
                                    ->with('typealert', 'danger');
                                endif;
                        ...
                        if($query == 0):
                            ...
                            if($oitem->save()):
                                ...
                                return back()->with('messages', '¡Producto agregado al carrito de compras!.')
                                    ->with('typealert', 'success');
                            endif;
                        else:
                            return back()->with('messages', 'Este producto ya se encuentra en el carrito de compras.')
    ...
    public function postCartItemQuantityUpdate($id, Request $request){
        ...
        if($order->id != $oitem->order_id):
            return back()->with('messages', 'No podemos actualizar la cantidad de este producto.')
                ->with('typealert', 'danger');
        else:
            if($inventory->limited == "0"):
                if($request->input('quantity') > $inventory->quantity ):
                    return back()->with('messages', 'La cantidad ingresada supera a nuestro inventario en estos momentos.')
                        ->with('typealert', 'danger');
                endif;
            endif;
            ...
            if($oitem->save()):
                ...
                return back()->with('messages', '¡Cantidad actualizada con exito!.')
                    ->with('typealert', 'success');
            endif;
        endif;
    }
    ...
    public function getCartItemDelete($id){
        ...
        if($oitem->delete()):
            ...
            return back()->with('messages', '¡Producto eliminado del carrito con exito!.')
                ->with('typealert', 'success');
        endif;
    }
