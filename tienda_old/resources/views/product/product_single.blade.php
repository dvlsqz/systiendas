                    ...
                    <div class="col-md-8">
                        ...
                        <div class="category">
                            <ul> 
                                ...
                                <li><span class="next"><i class="fas fa-chevron-right"></i></span></li>
                                <li><a href="{{ url('/store') }}"><i class="fas fa-tags"></i> {{ $product->cat->name }} </a></li>
                                @if($product->subcategory_id !=0 )
                                    <li><span class="next"><i class="fas fa-chevron-right"></i></span></li>
                                    <li><a href="{{ url('/store') }}"><i class="fas fa-tag"></i> {{ $product->getSubCategory->name }} </a></li>
                                @endif
                            </ul>
                        </div>
                                        ...
                                        <div class="variants mtop16" >
                                            ...
                                            <ul id="inventory">
                                                @foreach($product->getInventory as $inventory)
                                                    <li> 
                                                        <a href="#" class="inventory" id="inventory_{{ $inventory->id }}" data-inventory-id="{{ $inventory->id }}"> 
                                                            {{ $inventory->name }} -
                                                            <span class="price">
                                                                {{Config::get('cms.currency').number_format($inventory->price, 2, '.' ,',') }}
                                                            </span>
                                                        </a>                                                        
                                                    </li>
                                                @endforeach
